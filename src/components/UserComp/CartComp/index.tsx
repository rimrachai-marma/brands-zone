...
const CartComp = () => {
  ...
  const handleDecreaseQuantity = (item: (typeof items)[number]) => {
    if (item.quantity > 1) {
      ...
      toast.error(`"${item.title}" can not be less than 1.`);
    }
  };

  const handleIncreaseQuantity = (item: (typeof items)[number]) => {
    dispatch(addToCart({ product: item as unknown as Product }));
  };

  const handleRemoveFromCart = (itemId: number, itemTitle: string) => {
  ...
  if (!items || items.length === 0)
    return (
      <div className="flex flex-col items-center justify-center p-8 md:p-20 text-center bg-gray-50 min-h-[40vh] m-4 md:m-10 shadow-lg rounded-lg">
  ...
  return (
    <section className="py-6 md:py-12 px-4">
      <div className="container mx-auto">
        ...
        <div className="hidden md:block overflow-x-auto border shadow-lg border-primary/20 rounded-lg">
          <Table>
                ...
                >
                  <TableCell className="py-4">
                    <div className="flex items-center gap-4">
                      {item.image ? (
                        <div className="w-16 h-16 relative shrink-0 border overflow-hidden rounded">
                          <Image
                            src={item.image}
                            alt={item.title}
                            fill
