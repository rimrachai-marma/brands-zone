...
import {
  HeroSectionFullData,
  heroSectionFullSchema,
} from "@/schema/hero-section";
...
export default function HeroSectionForm({ initialData }: HeroSectionFormProps) {
  ...
  const form = useForm<HeroSectionFullData>({
    ...
    defaultValues: initialData
      ? {
          ...
          is_active: initialData.is_active ? 1 : 0, // Convert to number
          image_1: undefined,
          image_2: undefined,
          image_3: undefined,
  ...
  const handleImageChange = (fieldName: string, file?: File) => {
    ...
    const hadInitialImage =
      initialData?.[
        `full_image_${fieldName.split("_")[1]}` as keyof HeroSection
      ];
  ...
  const onSubmit = async (data: HeroSectionFullData) => {
    ...
    const toastId = toast.loading(
      initialData ? "Updating hero section..." : "Creating hero section..."
    );
    ...
    try {
      ...
      Object.entries(data).forEach(([key, value]) => {
        ...
        if (key.startsWith("image_")) {
          ...
          // Handle is_active - ensure it's sent as "1" or "0"
          formData.append(key, value ? "1" : "0");
          console.log(`Appending is_active: ${value ? "1" : "0"}`);
        } else {
          // Handle text fields
      ...
      let result;
      if (initialData) {
        console.log("Updating existing hero section, ID:", initialData.id);
